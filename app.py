...
if not df_total.empty:
    ...
    if not d_l.empty and not d_v.empty:
        ...
        # Ambos Marcan
        prob_ambos = (prob_1_loc/100 * prob_1_vis/100) * 100
        ...
        with c1:
            ...
            st.write(f"1: {p_win_l*100:.1f}% | X: {p_empate*100:.1f}% | 2: {p_win_v*100:.1f}%")

        with c2:
            st.subheader("🛡️ Doble Op.")
            st.info(f"**1X:** {(p_win_l + p_empate)*100:.1f}%")
            st.info(f"**X2:** {(p_win_v + p_empate)*100:.1f}%")
        ...
        with g1:
            ...
            st.markdown(f"<p style='{col_g_l}'>{prob_1_loc:.1f}%</p>", unsafe_allow_html=True)
        with g2:
